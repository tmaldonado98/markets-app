...
export default function StockData(props:any){	
		...
		const loopedDataPoints:any = [[], {param: []}];
		props.dataPoints.timestamp.map((each:any) => (loopedDataPoints[0].push(each)))
		
		let title = ''
		if(props.param === 'Highs'){
			props.dataPoints.indicators.quote[0].high.map((each:any)=> loopedDataPoints[1].param.push(each))
			title = props.param + " (in "+ curr +")"
		}
		else if(props.param === 'Lows'){
			props.dataPoints.indicators.quote[0].low.map((each:any)=> loopedDataPoints[1].param.push(each))
			title = props.param + " (in "+ curr +")"
		}
		else if(props.param === 'Open'){
			props.dataPoints.indicators.quote[0].open.map((each:any)=> loopedDataPoints[1].param.push(each))
			title = props.param + " (in "+ curr +")"
		}
		else if(props.param === 'Close'){
			props.dataPoints.indicators.quote[0].close.map((each:any)=> loopedDataPoints[1].param.push(each))
			title = props.param + " (in "+ curr +")"
		}
		else if(props.param === 'Volume'){
			props.dataPoints.indicators.quote[0].volume.map((each:any)=> loopedDataPoints[1].param.push(each))
			title = props.param
		}
		...
		else if (props.dataPoints.meta.currency === 'HKD') {
			options.axisY.prefix = '$'
			options.data[0].yValueFormatString = "$#,###"
